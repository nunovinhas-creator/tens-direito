...
| **Antes de 1 ago 2026** | Publicar o calendário de **agosto** — Fases 3+4 já implementadas a 12/07 (workflow `calendario-mensal.yml` vigia, abre Issue `calendario-manual` com prompt pronto, e vira a página no dia 1 quando o JSON tem o mês novo); falta só a sessão manual que confirme as datas de agosto assim que forem divulgadas (a fonte oficial pública desapareceu com a migração do portal — achado em `docs/FONTE-CALENDARIO.md`; triangular fontes que reproduzam o calendário oficial) | `calendario-pagamentos-seguranca-social.html` + `data/calendario_pagamentos.json` — ver CLAUDE.md **"CALENDÁRIO DE PAGAMENTOS"** |
